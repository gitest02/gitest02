...
if (!JsNamespace.exist("Iject"))
{
	...
	JsNamespace.declare("Iject", {
		...
		appOnload : function(o)
		{
			...
			Iject.$["framesize"]      = "0,*,60";  		         							  //defualt seperatesize
			...
			Iject.$["Board"]	= "Pattern:V_BOD_001.xfdl";    //board url setting
		...
		setSeprateFrame : function()
		{
			...
			//**** 임시 필요에따라 삭제해서 사용
			Iject.$["userid"] = application.gds_userInfo.getColumn(1,1);
			Iject.$["userpw"] = application.gds_userInfo.getColumn(1,2);
			//application.gds_userInfo.getColumn(0,"USER_NM");
